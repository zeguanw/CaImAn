...
# %%
def volpy_process_session(work_dir, layer_name):
    ...
    #fnames = os.path.join(work_dir + layer_name + '.tif' );
    fnames = os.path.join(work_dir + '/raw/' + layer_name + '.hdf5' );
...
#%% dataset dependent parameters
    ...
    max_deviation_rigid = 8                         # maximum deviation allowed for patch with respect to rigid shifts
    border_nan = 'copy'
...
# %%% MOTION CORRECTION
    ...
    if do_motion_correction:
        ...
        savefig(work_dir +'/auto/outline/' + layer_name + '_motion-correct.pdf', bbox_inches='tight')
...
# %% SEGMENTATION
    ...
    if method == 'maskrcnn':                 
        ...
        ROIs = utils.mrcnn_inference(img=summary_images.transpose([1, 2, 0]), size_range=[3, 18],
                                     weights_path=weights_path, display_result=True, savedir=work_dir, savename=layer_name) # size parameter decides size range of masks to be selected
        cm.movie(ROIs).save(work_dir + '/auto/volpy_roi/' + layer_name + '_mrcnn_ROIs.hdf5')
...
#vpy.estimates['t']
    ...
    sio.savemat(os.path.join(work_dir + '/auto/matvars/T/T_' + layer_name + '.mat'), mdict={'T': vpy.estimates['t']})
    sio.savemat(os.path.join(work_dir + '/auto/matvars/spikes/spikes_' + layer_name + '.mat'), mdict={'spikes': vpy.estimates['spikes']})
    sio.savemat(os.path.join(work_dir + '/auto/matvars/threshold/thresh_' + layer_name + '.mat'), mdict={'thresh': vpy.estimates['thresh']})
    sio.savemat(os.path.join(work_dir + '/auto/matvars/snr/snr_' + layer_name + '.mat'), mdict={'snr': vpy.estimates['snr']})
    sio.savemat(os.path.join(work_dir + '/auto/matvars/t_sub/t_sub_' + layer_name + '.mat'), mdict={'t_sub': vpy.estimates['t_sub']})
    
    cm.movie(vpy.estimates['weights']).save(os.path.join(work_dir + '/auto/matvars/weights/weights_' + layer_name + '.hdf5'))
    # sio.savemat(os.path.join(work_dir + '/matvars/T_' + layer_name + '.mat'), mdict={'T': vpy.estimates['t']})
    # sio.savemat(os.path.join(work_dir + '/matvars/spikes_' + layer_name + '.mat'), mdict={'spikes': vpy.estimates['spikes']})
